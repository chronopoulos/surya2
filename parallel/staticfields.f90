   ...
   subroutine define_mc()
      ...
      do j=1,2*n+1
         psi_mid(i,j) = psi_0 * (r_mid(i)-r_p) * dsin( pi*(r_mid(i)-r_p)/(rsun-r_p) ) &
                     * (1.-dexp(-beta_1*theta_mid(j)**epsi))*(1.-dexp(beta_2*(theta_mid(j)-pi/2.)))*dexp(-((r_mid(i)-r_0)/big_gamma)**2) &
                     / (r_mid(i)*dsin(theta_mid(j)))
      enddo ! j
      enddo ! i
   ...
   subroutine define_matrix_coefficients()
      ...
      do j=1,n
         ...
         ! matrix coefficients
            ...
            ba(i,j) = ( eta_a(i,j)*( 2./(r(i)*dth)**2 + 1./(dtan(theta(j))*r(i)**2*dth) + 1./(2.*r(i)**2*dsin(theta(j))**2) ) &
                        + utheta_ij/dth*( &
                          sinplus *(0.5 + dt/(4.*dth)*utheta_jplus *dsin(theta(j)))  &
                        - sinminus*(0.5 - dt/(4.*dth)*utheta_jminus*dsin(theta(j)))  &
                                         ) ) *dt/2.

            ca(i,j) = -( eta_a(i,j)*( 1./(r(i)*dth)**2  + 1./(dtan(theta(j))*r(i)**2*dth) ) &
                        - ur_ij*sinplus/dth &
                        * (0.5 - dt/(4.*dr)*v_theta_mid(imid,jmid+1)/(r(i))) ) *dt/2.
            ...
            eb(i,j) = ( eta_b(i,j)*( 2./(r(i)*dth)**2 + 1./(dtan(theta(j))*r(i)**2*dth) + 0.5/(r(i)*dsin(theta(j)))**2) &
                        + v_theta_mid(imid,jmid+1)/(r(i)*dth)*(0.5+dt/(4.*dth)*utheta_ij*dsin(theta(j))) &
                        - v_theta_mid(imid,jmid-1)/(r(i)*dth)*(0.5-dt/(4.*dth)*utheta_ij*dsin(theta(j))) &
                              ) *dt/2.

            fb(i,j) = -( eta_b(i,j)*( 1./(r(i)*dth)**2 + 1./(dtan(theta(j))*r(i)**2*dth) ) &
                        - v_theta_mid(imid,jmid+1)/(r(i)*dth)*( 0.5 - dt/(4.*dr)*utheta_ij*dsin(theta(j)) ) ) *dt/2.

            ab(i,j) = -( eta_b(i,j)/dr**2 + urbar_ij/dr*r_mid(imid-1)*( 0.5 + dt/(4.*dr)*urbar_iminus*r_mid(imid-1) ) ) *dt/2.

            bb(i,j) = ( eta_b(i,j)*( 2./dr**2 + 2./(r(i)*dr) + 0.5/(r(i)*dsin(theta(j)))**2 ) + urbar_ij &
                        + urbar_ij/dr*( dr/2. + r(i)*dt/(4.*dr)*(urbar_iplus*r_mid(imid+1)+urbar_iminus*r_mid(imid-1)) ) ) *dt/2.

            cb(i,j) = eta_b(i,j)*( 1./dr**2 + 1./(r(i)*dr) ) - urbar_ij/dr*r_mid(imid+1)*(0.5 - dt/(4.*dr)*urbar_iplus*r_mid(imid+1))
